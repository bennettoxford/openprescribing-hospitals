...
from bq_tables import ORG_AE_STATUS_TABLE_SPEC
...
def fetch_ae_data() -> pd.DataFrame:
    ...
    logger.info(f"Found {total_links} years of data to process")
    ...
    for year_index, link in enumerate(links, 1):
        ...
        for csv_index, csv_link in enumerate(csv_links, 1):
            ...
            columns = ["Org Code", "A&E attendances Type 1"] if year_month >= pd.to_datetime("2020-09") else ["Org Code", "Number of A&E attendances Type 1"]
            ...
            # Log progress at start, every 6 months, and at the end of each year
            if csv_index == 1 or csv_index % 6 == 0 or csv_index == total_csvs:
                logger.info(f"Year {year_index}/{total_links}: Processed {csv_index}/{total_csvs} months")
...
def prepare_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Convert timestamp to date by taking first day of month
...
def upload_to_bigquery(df: pd.DataFrame) -> None:
    ...
    job = client.load_table_from_dataframe(
        df, 
        ORG_AE_STATUS_TABLE_SPEC.full_table_id, 
        job_config=job_config
    )
    job.result()
    
    logger.info(f"Loaded {job.output_rows} rows into {ORG_AE_STATUS_TABLE_SPEC.full_table_id}")
