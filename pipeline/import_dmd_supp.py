...
from config import PROJECT_ID, DATASET_ID, DMD_SUPP_TABLE_ID
...
def parse_xml_data(xml_path: Path) -> list[dict]:
    ...
    for vmp in root.findall('.//VMP'):
        ...
        data.append({
            ...
            'ddd': ddd.text if ddd is not None else None,
            'ddd_uom': ddd_uom.text if ddd_uom is not None else None
        })
...
def upload_to_bigquery(data: list[dict]):
    ...
    logger = get_run_logger()
    full_table_id = f"{PROJECT_ID}.{DATASET_ID}.{DMD_SUPP_TABLE_ID}"
    logger.info(f"Uploading {len(data)} records to {full_table_id}")
    ...
    try:
        ...
        raise Exception(f"Table {full_table_id} does not exist. Please run setup_bq_tables.py first")
...
def import_dmd_supp_flow():
    ...
    try:
        ...
        logger.info(f"Successfully imported {rows_loaded} rows of dm+d supplementary data")
        return rows_loaded
