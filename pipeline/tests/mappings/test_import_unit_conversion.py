...
from pipeline.mappings.import_unit_conversion import (
    import_mapping,
)
...
class TestImportUnitConversion:
    ...
    def mock_bigquery_client(self):
        with patch('pipeline.mappings.import_unit_conversion.get_bigquery_client') as mock:
            client = Mock()
    ...
    def test_import_mapping(self, mock_bigquery_client, sample_units_dict):
        """Test the import_unit_conversion task"""
        table_id = f"{PROJECT_ID}.{DATASET_ID}.{UNITS_CONVERSION_TABLE_ID}"
        
        import_mapping(table_id, sample_units_dict)
