...
from pipeline.flows.import_ddd import (
    split_routes,
    validate_who_routes,
    create_ddd_mappings,
    process_ddd_data,
...
class TestWHORoutesValidation:
    ...
    def mock_bigquery_client(self):
        with patch('pipeline.flows.import_ddd.get_bigquery_client') as mock:
            client = Mock()
...
class TestFetchDDDAlterations:
    @patch('pipeline.flows.import_ddd.fetch_table_data_from_bq')
