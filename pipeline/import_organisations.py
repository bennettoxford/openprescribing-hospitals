...
from config import PROJECT_ID, DATASET_ID, ORGANISATION_TABLE_ID
...
def fetch_all_trusts_from_ord() -> List[str]:
    ...
    roles = ["RO197", "RO24"]
    urls = [f"https://directory.spineservices.nhs.uk/ORD/2-0-0/organisations?Roles={role}&Limit=1000" for role in roles]
    all_orgs = []
    for url in urls:
        try:
    ...
    for i, org in enumerate(orgs, 1):
        ...
        try:
            ...
            if i % 50 == 0 or i == total:
                logger.info(f"Progress: {i}/{total} organisations processed ({(i/total)*100:.1f}%)")
        except requests.RequestException as e:
...
def upload_to_bigquery(client: bigquery.Client, df: pd.DataFrame, full_table_id: str) -> None:
...
def create_org_mapping_df(
    ...
    for org, org_details in orgs_details.items():
        ...
        org_details_dict = {
            "ods_code": org,
            "ods_name": format_org_name(org_details.get("Organisation", {}).get("Name", "")),
            ...
            "postcode": org_details.get("Organisation", {}).get("GeoLoc", {}).get("Location", {}).get("PostCode", None),
            "region": region_name,
            "icb": icb_name
        }

        org_mapping_df = pd.concat(
            [org_mapping_df, pd.DataFrame([org_details_dict])], 
            ignore_index=True
            )
...
def import_organisations():
    ...
    organisation_df = create_org_mapping_df(successors, predecessors, all_orgs_details, icbs, icb_regions, region_names, icb_names)

    upload_to_bigquery(client, organisation_df, table_id)
