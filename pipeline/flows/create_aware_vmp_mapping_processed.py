...
from pipeline.bq_tables import AWARE_VMP_MAPPING_PROCESSED_TABLE_SPEC
...
def create_aware_vmp_mapping_processed_table():
    ...
    try:
        result = execute_bigquery_query_from_sql_file('pipeline/sql/create_aware_vmp_mapping_processed.sql')
...
def create_aware_vmp_mapping_processed_flow():
    """
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Failed to create processed AWaRe VMP mapping table: {str(e)}",
            "creation": None,
            "validation": None
        }
...
if __name__ == "__main__":
    result = create_aware_vmp_mapping_processed_flow()
