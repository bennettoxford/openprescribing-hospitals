...
def table_has_data(table_name: str) -> bool:
    ...
    try:
        ...
        logger.info(f"Table {table_id} has {count} rows")
        return count > 0
    except NotFound:
        raise RuntimeError(f"Table {table_id} does not exist but should!")

@task
def validate_who_routes(df: pd.DataFrame, client: bigquery.Client):
...
def import_ddd_atc_flow():
    ...
    atc_has_data = table_has_data("who_atc")
    ddd_has_data = table_has_data("who_ddd")
    ...
    try:
        atc_path = download_from_gcs(
            BUCKET_NAME,
            "who_atc_ddd_op_hosp/2024 ATC.xml",
            temp_dir
        )
        ddd_path = download_from_gcs(
            BUCKET_NAME,
            "who_atc_ddd_op_hosp/2024 ATC_ddd.xml",
            temp_dir
        )
        ...
        client = get_bigquery_client()
        validate_who_routes(ddd_df, client)
        
        if not atc_has_data:
            load_to_bigquery(atc_df, "who_atc")
        if not ddd_has_data:
            load_to_bigquery(ddd_df, "who_ddd")
