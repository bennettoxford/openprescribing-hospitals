...
-- Enhanced ingredient_data to handle multi-ingredient "refers to" cases
-- and expressed_as strength adjustments
ingredient_data AS (
  SELECT
    iq.year_month,
    iq.ods_code,
    ...
    -- Check if VMP has expressed_as adjustment from mapping table
  ...
  SELECT
    *,
    CASE
      ...
      -- When using ingredient quantity (works for all cases including expressed_as)
      WHEN can_calculate_ddd 
        AND selected_ddd_basis_value > 0 
