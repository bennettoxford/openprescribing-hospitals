...
ingredient_matches AS (
  ...
  INNER JOIN dmd_ingredients di 
    ON LOWER(di.dmd_ingredient_name) LIKE CONCAT('%', LOWER(TRIM(dc.refers_to_ingredient)), '%')
  WHERE dc.refers_to_ingredient IS NOT NULL

  UNION ALL

  -- Special case: "alendronic acid" should also match ingredients containing "alendronate"
  SELECT 
    dc.ddd_comment,
    dc.refers_to_ingredient,
    di.dmd_ingredient_code,
  ...
  FROM ddd_comments_with_refers_to dc
  INNER JOIN dmd_ingredients di 
    ON LOWER(TRIM(dc.refers_to_ingredient)) = 'alendronic acid'
