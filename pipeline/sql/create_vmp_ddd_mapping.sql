...
-- Add CTE to get "refers to" ingredient mappings
...
ddd_route_selection AS (
  SELECT
    ...
    -- Updated to allow "Refers to" and "Expressed as" comments
    CASE
...
-- Pre-calculate "refers to" ingredient names to avoid correlated subqueries
...
unit_and_ingredient_analysis AS (
  SELECT
    ...
    -- Get the specific ingredient name for "refers to" cases (using pre-calculated data)
