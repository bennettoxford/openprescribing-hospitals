...
from config import PROJECT_ID, DATASET_ID, UNITS_CONVERSION_TABLE_ID
...
def import_unit_conversion(client: bigquery.Client, table_id: str, units_dict: Dict) -> List[Dict]:
    ...
    for i, (unit, values) in enumerate(units_dict.items(), 1):
        ...
        if i % 50 == 0 or i == total_units:
            logger.info(f"Progress: {i}/{total_units} units processed ({(i/total_units)*100:.1f}%)")
    ...
    job.result()
    logger.info(f"Successfully imported {len(conversion_records)} unit conversion records to {table_id}")

@flow(name="Import Unit Conversion")
def import_unit_conversion_flow():
    client = get_bigquery_client()
    table_id = f"{PROJECT_ID}.{DATASET_ID}.{UNITS_CONVERSION_TABLE_ID}"
    units_dict = create_units_dict()
    import_unit_conversion(client, table_id, units_dict)
