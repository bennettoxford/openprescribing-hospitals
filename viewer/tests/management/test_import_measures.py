...
class TestImportMeasures:
    ...
    def test_validate_measure_yaml_valid_y_axis_label(self, valid_measure_data):
        valid_measure_data['y_axis_label'] = 'Units prescribed'
        validate_measure_yaml(valid_measure_data)
...
"""
    ...
    @pytest.mark.django_db
    def test_command_imports_y_axis_label_when_present(self, tmp_path, measure_tags):
        measures_dir = tmp_path / 'measures'
        measures_dir.mkdir()

        test_measure_dir = measures_dir / 'test-measure-y-axis'
        test_measure_dir.mkdir()
        ...
        yaml_content = f"""
name: Test Measure Y Axis
short_name: test-measure-y-axis
description: Test description
why_it_matters: Test why it matters
how_is_it_calculated: Test calculation method
tags: ['test-tag-1']
quantity_type: dose
y_axis_label: Custom axis label
authored_by: Test Author
checked_by: Test Checker
date_reviewed: '{today}'
next_review: '{next_review}'
...
"""
        ...
        with patch('viewer.management.commands.import_measures.Path') as mock_path:
            mock_path.return_value.parent.parent.parent = tmp_path
            call_command('import_measures', 'test-measure-y-axis')

        measure = Measure.objects.get(short_name='test-measure-y-axis')
        assert measure.y_axis_label == 'Custom axis label'
