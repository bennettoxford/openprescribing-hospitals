...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(
            "Successfully loaded all dummy data into the database"))
    ...
    def load_vmp(self, directory):
        ...
        VMP.objects.bulk_create(
            [VMP(code=vmp["code"], name=vmp["name"], vtm_id=vmp["vtm"]) for vmp in vmps]
        )
    ...
    def load_organisation(self, directory):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Loaded {len(organisations)} Organisations")
        )
    ...
    def load_ingredient_quantity(self, directory):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Loaded {len(ingredient_quantities)} IngredientQuantities"
            )
        )
