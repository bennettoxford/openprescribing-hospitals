...
class Command(BaseCommand):
    help = "Creates a unit conversion table in BigQuery"
    ...
    def handle(self, *args, **options):
        ...
        conversion_table = pd.DataFrame(units_dict).T.reset_index()
        ...
        conversion_table.loc[conversion_table["unit"] == "insert_old", "unit"] = "insert"
        conversion_table.loc[
            conversion_table["unit"] == "unit dose old", "unit"
        ] = "unit dose"
        ...
        job = client.load_table_from_dataframe(
            conversion_table, table_id, job_config=job_config
        )
        job.result()

        self.stdout.write(self.style.SUCCESS(f"Loaded {len(conversion_table)} rows into {table_id}"))