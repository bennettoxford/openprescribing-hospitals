...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        vmps = [
            {
                "code": "42206511000001102",
                "name": "Apixaban 5mg tablets",
                "vtm": "VTM001",
            },
            {
                "code": "14254711000001104",
                "name": "Rivaroxaban 10mg tablets",
                "vtm": "VTM002",
            },
            {
                "code": "38728311000001104",
                "name": "Dabigatran etexilate 150mg capsules",
                "vtm": "VTM003",
            },
        ]
        ...
        organisations = [{"ods_code": "RK9",
                          "ods_name": "University Hospitals Plymouth NHS Trust",
                          "region": "South West",
                          },
                         {"ods_code": "RT2",
                          "ods_name": "Pennine Care NHS Foundation Trust",
                          "region": "North West",
                          },
                         {"ods_code": "RXY",
                          "ods_name": "Kent And Medway NHS And Social Care Partnership Trust",
                          "region": "South East",
                          },
                         ]
        ...
        self.generate_dose_csv(
            start_date, end_date, vmps, organisations, dummy_data_dir
        )
        self.generate_scmd_csv(
            start_date, end_date, vmps, organisations, dummy_data_dir
        )
        self.generate_ingredient_quantity_csv(
            start_date,
            end_date,
            ingredients,
            vmps,
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully generated all dummy data CSV files in {dummy_data_dir}"
            )
        )
    ...
    def generate_csv(self, filename, fieldnames, data, directory):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Generated {filename}.csv in {directory}")
        )
    ...
    def generate_ingredient_csv(self, ingredients, directory):
        self.generate_csv(
            "ingredient", [
                "code", "name"], ingredients, directory)

    def generate_organisation_csv(self, organisations, directory):
        self.generate_csv(
            "organisation", [
                "ods_code", "ods_name", "region"], organisations, directory)

    def generate_dose_csv(
            self,
            start_date,
            end_date,
            vmps,
        ...
        self.generate_csv(
            "dose",
            ["year_month", "vmp", "quantity", "unit", "organisation"],
            doses,
            directory,
        )

    def generate_scmd_csv(
            self,
            start_date,
            end_date,
            vmps,
    ...
    def generate_ingredient_quantity_csv(
        self, start_date, end_date, ingredients, vmps, organisations, directory
    ):
        ...
        self.generate_csv(
            "ingredient_quantity",
            ["year_month", "ingredient", "vmp", "quantity", "unit", "organisation"],
            ingredient_quantities,
            directory,
        )
