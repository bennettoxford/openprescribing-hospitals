...
#### 1. Main Analysis Data CSV (`{filename}_data.csv`)
...
#### 2. Percentiles Data CSV (`{filename}_percentiles.csv`)
...
- **Date**: The month for which percentiles are calculated. e.g. rows where the *Date* is `2020-01-01` represent a percentile value, calculated using the data for all included NHS Trusts in January 2020.
- ***th_percentile**: Percentile values for the selected quantity across all trusts.

#### 3. Trusts Summary CSV (`{filename}_trusts_summary.csv`)

This file lists all NHS Trusts in scope for the analysis, with their total issuing quantity over the analysis period. Trusts with no data for the selected products are included with a total quantity of zero, so you can see which trusts had no activity for the chosen products.
