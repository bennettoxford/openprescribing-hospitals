...
measure_data AS (
    ...
    WHERE ingredient_quantity.year_month BETWEEN (SELECT MIN(year_month) FROM viewer_ingredientquantity) AND (SELECT MAX(year_month) FROM viewer_ingredientquantity)
        AND vtm.vtm IN ('774624002', '777455008', '775732007', '13568411000001103')
    GROUP BY
...
vmp_lists AS (
    SELECT 
        array_agg(DISTINCT vmp.code) FILTER (WHERE vtm.vtm IN ('774624002', '777455008')) AS numerator_vmps,
